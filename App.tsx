...
const App: React.FC = () => {
  ...
  const validate = (checkEmail = false): boolean => {
    ...
    if (newErrors.length > 0) {
      setShowToast({ msg: "Por favor corrige los campos faltantes", type: 'error' });
  ...
  const handleGenerateAiConcept = async () => {
    ...
    try {
      ...
      if (response.text) {
        ...
        setShowToast({ msg: "IA optimizó el texto", type: 'success' });
        setTimeout(() => setShowToast(null), 2000);
      }
    } catch (err) {
      setShowToast({ msg: "Error con la IA", type: 'error' });
    } finally {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      setShowToast({ msg: "Error al imprimir", type: 'error' });
    }
  };
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setShowToast({ msg: "¡Email enviado con éxito!", type: 'success' });
      ...
      setShowToast({ msg: err.message || "Error al enviar", type: 'error' });
    ...
    <div className="min-h-screen flex flex-col bg-[#F8FAFC]">
      ...
      <main className="flex-1 container mx-auto p-4 lg:p-8 flex flex-col lg:flex-row gap-8">
        <div className="w-full lg:w-[45%] order-1">
          <div className="mb-8">
             <h2 className="text-2xl font-black text-slate-900 tracking-tight">Datos del Cobro</h2>
             <p className="text-slate-500 text-sm">Completa la información para generar el documento.</p>
          </div>
          ...
          {errors.length > 0 && (
            <div className="mt-6 p-5 bg-rose-50 border border-rose-100 rounded-3xl animate-pulse">
               <h4 className="text-rose-800 font-black text-xs uppercase tracking-widest mb-2 flex items-center gap-2">
                 <i className="fas fa-triangle-exclamation"></i> Campos por corregir
               </h4>
               <ul className="space-y-1">
                 {errors.map((err, i) => (
                   <li key={i} className="text-rose-600 text-[11px] font-bold list-disc ml-4">{err}</li>
                 ))}
               </ul>
            </div>
          )}
        ...
        <div className="w-full lg:w-[55%] order-2">
          <div className="sticky top-24">
            <h2 className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.2em] mb-4 px-2">Previsualización Profesional</h2>
      ...
      <footer className="bg-white border-t border-slate-200 py-8 text-center text-slate-400 text-[10px] font-bold uppercase tracking-widest">
        AXYRA SOLUTIONS &copy; {new Date().getFullYear()} | INFRAESTRUCTURA DE COBROS COLOMBIA
      </footer>
    </div>
  );
};
