...
const InvoiceForm: React.FC<Props> = ({ state, onUpdate, onClear, onDownload, onPrint, onSend, onAiConcept, status, isAiGenerating, errors }) => {
  ...
  const formatDateForInput = (dateStr: string): string => {
    if (!dateStr) return '';
    // Handle "YYYY-MM-DD" format (ISO) by converting to input-friendly format
    // Input type="date" expects "YYYY-MM-DD" which matches ISO format
    return dateStr;
  };
    ...
    <div className="space-y-6 pb-28">
      <FormCard title="Emisor (Tus Datos)" icon="fa-fingerprint">
        <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-50">
           <p className="text-[9px] text-slate-400 font-bold uppercase">Datos de facturaciÃ³n fijos</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Input label="Tu TelÃ©fono" value={state.myData.telefono} onChange={(v) => handleChange('myData', 'telefono', v)} disabled={!state.editMyData} />
          <Input label="Tu DirecciÃ³n" value={state.myData.direccion} onChange={(v) => handleChange('myData', 'direccion', v)} disabled={!state.editMyData} />
        </div>
      </FormCard>
      ...
      <FormCard title="Cliente / Deudor" icon="fa-user-tie">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Input label="NIT / CÃ©dula" value={state.clientData.nit} onChange={(v) => handleChange('clientData', 'nit', v)} error={hasError('nit')} />
          <div className="md:col-span-2">
            <Input label="Email de NotificaciÃ³n" type="email" value={state.clientData.email} onChange={(v) => handleChange('clientData', 'email', v)} error={hasError('email')} />
          </div>
        </div>
      </FormCard>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2 space-y-2">
             <div className="flex justify-between items-center">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">DescripciÃ³n de Servicios</label>
          ...
          <div className="md:col-span-2 space-y-3 pt-2 border-t border-slate-100">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Fecha de EmisiÃ³n</label>
                <input
                  type="date"
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-900 focus:border-blue-600 outline-none transition-all mt-1"
                  value={formatDateForInput(state.invoiceDetails.fechaEmision)}
                  onChange={(e) => handleChange('invoiceDetails', 'fechaEmision', e.target.value)}
                />
              </div>
              <div>
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Fecha de Vencimiento</label>
                <input
                  type="date"
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-900 focus:border-blue-600 outline-none transition-all mt-1"
                  value={formatDateForInput(state.invoiceDetails.fechaVencimiento)}
                  onChange={(e) => handleChange('invoiceDetails', 'fechaVencimiento', e.target.value)}
                />
              </div>
            </div>

            <div>
              <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Observaciones</label>
              <textarea
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-900 focus:border-blue-600 outline-none transition-all h-20 resize-none mt-1"
                value={state.invoiceDetails.observaciones}
                onChange={(e) => handleChange('invoiceDetails', 'observaciones', e.target.value)}
      ...
      <FormCard title="Datos Bancarios" icon="fa-university">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Input label="NÂº Cuenta" value={state.bankData.numero} onChange={(v) => handleChange('bankData', 'numero', v)} error={hasError('nÃºmero de cuenta')} />
          <Input label="Titular" value={state.bankData.titular} onChange={(v) => handleChange('bankData', 'titular', v)} />
        </div>
      </FormCard>

      {/* BARRA FLOTANTE REDISEÃ‘ADA (Slim Glass) */}
